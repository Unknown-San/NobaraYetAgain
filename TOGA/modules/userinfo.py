...
def info(update: Update, context: CallbackContext):
    ...
    if user_id not in [bot.id, 777000, 1087968824]:
        ...
        text += f"\n\n<b>• Health Level :</b> <code>{userhp['earnedhp']}/{userhp['totalhp']}</code>\n[<i>{make_bar(int(userhp['percentage']))} </i>{userhp['percentage']}%]"
...
def set_about_bio(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
        ...
        if user_id == bot.id and sender_id not in DEV_USERS:
            message.reply_text(
                "Erm... yeah, I only trust the ***** to set my bio.",
            )
            return
...
STATS_HANDLER = CommandHandler(["tsystem"], stats, run_async=True)
STATS1_HANDLER = CommandHandler(["tstats"], stats1, run_async=True)
